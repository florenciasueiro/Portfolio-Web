...
export function Contact() {
  ...
  const submit = async (data: FormData) => { const subject=encodeURIComponent(`Contacto de ${data.name} desde el portfolio`); const body=encodeURIComponent(`${data.message}\n\nEmail: ${data.email}`); window.location.href=`mailto:florencia.sueiro@davinci.edu.ar?subject=${subject}&body=${body}`; setSent(true); reset(); };
  return <Section id="contacto" eyebrow="06 / CONTACTO" title={<>Hagamos algo<br /><em>extraordinario.</em></>} className="contact-section"><div className="contact-grid"><div><p>Estoy buscando mi próxima oportunidad como desarrolladora. Si mi perfil encaja con tu equipo, me encantaría conversar.</p><a href="mailto:florencia.sueiro@davinci.edu.ar">florencia.sueiro@davinci.edu.ar</a></div><form onSubmit={handleSubmit(submit)} noValidate>
    ...
    <Button type="submit" disabled={isSubmitting}>{sent ? <><CheckCircle2 size={17}/> Listo</> : <>Enviar mensaje <Send size={17}/></>}</Button>
  </form></div></Section>;
}
